...
bool Fmart::addStudent()
{
    ...
    {
        m_students[m_numStudents] = new Student(this, randInt(0, 2), 9);
        return true;
    }
    return false; // This line is only for the program to compile.
}
...
void Fmart::display()
{
    ...
    {
        disp[m_player->row()][m_player->col()] = 'P';
    }
    if(m_player->mealInHand() == APPLEPIE)
    {
        disp[m_player->row()][m_player->col()] = 'A';
    }
    if(m_player->mealInHand() == BEEF)
    {
        disp[m_player->row()][m_player->col()] = 'B';
    }
    if(m_player->mealInHand() == CURRY)
    {
        disp[m_player->row()][m_player->col()] = 'C';
    }
    for (int i = 0; i < 3; i++){
        for (int j = 0; j < 10; j++)
            cout << disp[i][j];
...
void Player::deliver()
{
    for(int i = 0; i< 10; i++)
    {
        if((m_fp->studentAt(m_row, i) == true)&&(m_mealInHand == m_fp->getStudentmeal(m_row, i)))
        {
            m_score += 10;
